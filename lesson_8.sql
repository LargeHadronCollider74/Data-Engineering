...
-- select *
-- from messages t1
-- where (10 = t1.to_user_id);

-- select t1.*, t2.*
-- from messages t1, friendship t2
-- where (t1.from_user_id = t2.user_id)
--    and (10 = t1.to_user_id);
  
-- select * from friendship_request_types;

-- select t1.*, t2.*
-- from messages t1, friendship t2, friendship_request_types t3
-- where (t1.from_user_id = t2.user_id)
--   and (t2.request_type_id = t3.id)
--   and ('approved' = t3.name)
--   and (10 = t1.to_user_id);
...
from (select t1.from_user_id
      ...
      where (t1.from_user_id = t2.user_id)
        ...
        and ('approved' = t3.name)
        and (10 = t1.to_user_id)
      group by t1.from_user_id) t1,
    users t2
where (t1.from_user_id = t2.id)
  and (1 = t1.`rank`);
...
from likes t1,
     (select t1.id
      ...
      where (t1.id = t2.user_id)
        and (10 > year(now()) - year(t2.birthday))) t2
where (t1.user_id = t2.id);

-- п.3
select t1.gender
from (select t2.gender
            ,count(*)
            ,DENSE_RANK() OVER (ORDER BY count(*) DESC)  `rank`
      from likes t1,
