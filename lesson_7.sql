...
INSERT INTO `flights` (`id`, `from`, `to`) VALUES (6, 'minsk', 'ny');
...
select *
from users t1
where exists (select * from orderss t2 where (t1.id = t2.user_id));

select *
from users t1
where (id in (select user_id from orderss group by user_id));

select *
from users t1
where (id in (select distinct user_id from orderss));

select t1.*
from users t1, (select distinct user_id from orderss) t2
...
-- п.3
select t1.id
      ,ifnull(t2.name, t1.`from`) `from`
      ,ifnull(t3.name, t1.`to`) `to`
from flights t1
left outer join cities t2 on (t1.`from` = t2.label)
left outer join cities t3 on (t1.`to` = t3.label);
